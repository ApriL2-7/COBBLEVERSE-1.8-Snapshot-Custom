...
for /f "usebackq delims=" %%P in (`powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "Add-Type -AssemblyName System.Windows.Forms; $root=Join-Path $env:APPDATA 'ModrinthApp\profiles'; $preferred=Join-Path $root 'COBBLEVERSE 1.8 Snapshot Custom'; if (Test-Path -LiteralPath $preferred -PathType Container) { $preferred } else { $guard='cobbleverse-client-pack-guard-2026.08.29.1.jar'; $matches=@(); foreach ($dir in @(Get-ChildItem -LiteralPath $root -Directory -ErrorAction SilentlyContinue)) { if (Test-Path -LiteralPath (Join-Path $dir.FullName ('mods\' + $guard)) -PathType Leaf) { $matches += $dir } }; if ($matches.Count -eq 1) { $matches[0].FullName } else { $dlg=[Windows.Forms.FolderBrowserDialog]::new(); $dlg.Description='COBBLEVERSE Modrinth profile folder'; if (Test-Path -LiteralPath $root -PathType Container) { $dlg.SelectedPath=$root }; if ($dlg.ShowDialog() -eq [Windows.Forms.DialogResult]::OK) { $dlg.SelectedPath } } }"`) do set "PROFILE_PATH=%%P"
